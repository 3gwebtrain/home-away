...
const getAuthUser = async (): Promise<User> => {
  ...
  if (!user) throw new Error('You must me logged in to access this page');
...
const renderError = (error: unknown): { message: string } => {
  console.log(error);
  return { message: error instanceof Error ? error.message : 'there was un error' };
};
...
export const createPropertyAction = async (
  ...
  try {
    ...
    await db.property.create({ data: { ...validatedFields, image: fullPath, profileId: user.id } });
  } catch (error) {
    return renderError(error);
  }
  redirect('/');
